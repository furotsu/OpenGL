...
Actor::~Actor(void)
{
    LOG("Actor", std::string("Destroying Actor ") + std::to_string(m_id));
    ASSERT(m_components.empty());  // [rez] if this assert fires, the actor was destroyed without calling Actor::Destroy()
}

bool Actor::Init(TiXmlElement* pData)
{
    LOG("Actor", std::string("Initializing Actor ") + std::to_string(m_id));
